...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.getElementById('psychologist-list') && !document.getElementById('agendamento-intro')) {
...
async function selectPsychologistFromStorage() {
    ...
    if (psychExists) {
        ...
        showFeedback('Psicólogo selecionado. Complete seus dados para finalizar o agendamento.', 'success');
    }
}
...
function loadPatientSession() {
    ...
    if (currentPatientEmail) {
        ...
        if (authMessage) {
            authMessage.textContent = 'Faça login com seu email para ver seus agendamentos.';
...
async function loadServices() {
    try {
        ...
        services = [
            { id: 1, nome: 'Terapia Individual', descricao: 'Sessão individual de acompanhamento', valor: 'R$ 120', modalidade: 'Presencial', ativo: true },
            { id: 2, nome: 'Terapia de Casal', descricao: 'Sessão para casal', valor: 'R$ 180', modalidade: 'Presencial', ativo: true },
            { id: 3, nome: 'Atendimento Online', descricao: 'Sessão online', valor: 'R$ 100', modalidade: 'Online', ativo: true },
            { id: 4, nome: 'Orientação Psicológica', descricao: 'Atendimento breve', valor: 'R$ 80', modalidade: 'Online', ativo: true },
        ];
    }
}
...
async function selectPsychologist(id) {
    ...
    document.getElementById('selected-psychologist-name').textContent = psychologists.find(p => p.id === id)?.nome || 'Nenhum';
    document.getElementById('confirm-psychologist').textContent = psychologists.find(p => p.id === id)?.nome || '-';
...
async function loadPsychologistSchedule(psychologistId) {
    try {
        const data = await fetchJson(`${API_URL}/psicologos/${psychologistId}/horarios`);
        scheduleEntries = data.map(item => ({
            ...
            label: item.label || `${item.data || item.date} ${item.hora || item.time}`,
        }));
    } catch (error) {
        scheduleEntries = [];
    }
        ...
        <div class="schedule-preview-grid">
            ${Object.keys(grouped).map(date => `
                <div class="schedule-day">
                    <div class="schedule-day-title">${new Date(date).toLocaleDateString('pt-BR')}</div>
                    <div class="schedule-times">
                        ${grouped[date].map(item => `
                            <button type="button" class="schedule-slot ${selectedSchedule?.id === item.id ? 'selected' : ''}" onclick="selectSchedule(${item.id})">${item.time}</button>
                        `).join('')}
                    </div>
                </div>
            `).join('')}
...
function selectSchedule(scheduleId) {
    ...
    renderSchedulePreview();
    if (selectedSchedule) {
        showFeedback(`Horário selecionado: ${selectedSchedule.date} ${selectedSchedule.time}`, 'success');
    }
}
...
function updateSelectedAppointmentDisplay() {
    ...
    if (confirmDate && confirmTime) {
        confirmDate.textContent = selectedSchedule ? new Date(selectedSchedule.date).toLocaleDateString('pt-BR') : '-';
        confirmTime.textContent = selectedSchedule ? selectedSchedule.time : '-';
    }

    if (confirmPsychologist) {
        confirmPsychologist.textContent = selectedPsychologistId ? psychologists.find(p => p.id === selectedPsychologistId)?.nome || '-' : '-';
    }
}
...
function showFeedback(message, type) {
    ...
    feedback.style.border = '1px solid ' + (type === 'error' ? '#f5c6cb' : '#c3e6cb');
...
function submitForm() {
    ...
    if (!selectedSchedule || !selectedPsychologistId) {
        showFeedback('Selecione um psicólogo e um horário válidos antes de enviar.', 'error');
        return;
    }

    const serviceId = parseInt(document.getElementById('servico').value, 10);
    const service = services.find(item => item.id === serviceId);
    const serviceName = service ? service.nome : document.getElementById('servico').value;
    const psychologist = psychologists.find(item => item.id === selectedPsychologistId);
...
function showConfirmation(data) {
    const content = `
        ...
        <p><strong>Psicólogo:</strong> ${data.psicologo_name || 'Não informado'}</p>
...
function loadAppointments() {
    ...
    const userEmail = currentPatientEmail || localStorage.getItem('patient_email');
